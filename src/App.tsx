  ...
  if (rawText && rawText.trim().length > 0) {
    if (contentType.includes("application/json") || rawText.trim().startsWith("{") || rawText.trim().startsWith("[")) {
      try {
        ...
        console.warn(`[safeFetchJson] Failed to parse JSON response from ${url}:`, rawText.slice(0, 120));
        throw new Error(res.ok ? "Invalid JSON response received from server." : `Server returned HTTP ${res.status}: ${res.statusText}`);
      }
    } else {
      if (!res.ok) {
...
export const quickTopics = [
  { label: "💰 Personal Finance Advice", prompt: "Can you give me some practical personal finance advice and budgeting tips?" },
  { label: "📈 Tax Deductions Explained", prompt: "Explain common tax deductions and smart filing strategies in simple terms." },
  { label: "💻 Tech: PC Build vs M-Mac", prompt: "Compare building a custom PC vs buying an Apple M-series Mac for work and creativity." },
  { label: "🗺️ Geography & Travel Vibe", prompt: "What are some hidden gem travel destinations with incredible culture and landscapes?" },
  { label: "🌱 Stress/Emotional Support", prompt: "I'm feeling a bit overwhelmed and stressed today. Can you offer some gentle advice and mindfulness guidance?" },
];
...
function App() {
  ...
  const handleResendOtp = useCallback(async () => {
    ...
    setResendSuccessMsg("");
    try {
      const data = await safeFetchJson<{ success?: boolean; error?: string }>("/api/auth/send-otp", {
        method: "POST",
  ...
  useEffect(() => {
    ...
    if (loggedInUser?.id) {
      safeFetchJson<{ success: boolean; sessions?: ChatSession[] }>("/api/history", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleGenerateIllustration = async () => {
    ...
    try {
      const data = await safeFetchJson<{ url?: string; error?: string }>("/api/transform-illustration", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            ...
            <div className="w-full text-left">
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  setAuthError("");
                  try {
                    const data = await safeFetchJson<{ user: any; token: string }>("/api/auth/login", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
            ...
            <div className="w-full text-left space-y-4">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  setResendSuccessMsg("");
                  try {
                    const data = await safeFetchJson<{ user: any; token: string }>("/api/auth/verify-otp", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    setAuthError("");
                    try {
                      await safeFetchJson<{ success?: boolean; error?: string }>("/api/auth/forgot-password", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    setResendSuccessMsg("");
                    try {
                      await safeFetchJson<{ success?: boolean; error?: string }>("/api/auth/verify-reset-otp", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    setAuthError("");
                    try {
                      await safeFetchJson<{ success?: boolean; error?: string }>("/api/auth/reset-password", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                          email: resetEmail,
                        ...
                        <div className="p-4 border-t border-[#EBE6DD] space-y-4">
                          ...
                          <button
                            ...
                            onClick={async () => {
                              ...
                              try {
                                const data = await safeFetchJson<{ user: any }>("/api/auth/update-profile", {
                                  method: "POST",
                                  headers: { "Content-Type": "application/json" },
                                  body: JSON.stringify({
                        ...
                        <div className="p-4 border-t border-[#EBE6DD] space-y-4">
                          ...
                          <button
                            ...
                            onClick={async () => {
                              ...
                              try {
                                const data = await safeFetchJson<{ token: string }>("/api/auth/change-password", {
                      ...
                      <form 
                        onSubmit={async (e) => {
                          ...
                          if (accountView === "create") {
                            ...
                            setAuthLoading(true);
                            try {
                              await safeFetchJson<{ success?: boolean; error?: string }>("/api/auth/send-otp", {
                                method: "POST", headers: { "Content-Type": "application/json" },
                                body: JSON.stringify({ fullName, nickname, email, password })
                              });
